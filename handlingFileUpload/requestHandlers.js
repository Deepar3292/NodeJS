...
function upload(response, request){
  ...
  form.uploadDir = "Penguins.jpg"; //change this path
  console.log("About to parse");
  form.parse(request,function(error,fields,files){
    console.log("Parsing done.");
    fs.rename(files.upload.path, "Penguins.jpg", function(error){ //change this path
      if(error){
        console.log("Error in uploading file.");
        fs.unlink("Penguins.jpg");  //change this path
        fs.rename(files.upload.path, "Penguins.jpg"); //change this path
      } 
    });
    response.writeHead(200,{"Content-Type":"text/html"});
...
function show(response){
  ...
  fs.readFile("Penguins.jpg", "binary", function(error,file){ //change this path
    if(error){
